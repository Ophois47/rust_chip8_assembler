...
// Transform Valid Expressions of Tokens into OpCodes
...
impl CodeGenerator {
	...
	// Consumes Code Gen and Expressions to Return Vector Containing Generated Operand
	pub fn generate(mut self, exprs: Vec<Expression>) -> Vec<u8> {
		...
		// Perform Second Pass of Expressions to Add Ones Unable to Complete
	...
	fn process_jump_instruction(&mut self, first: u8, expr: &Expression) {
        // Operand is Numeric Literal, Opcode Can be Generated
        ...
        // When Operand is a Label Operand
        if let Token::LabelOperand(ref label) = expr[1] {
            ...
            if self.labels.contains_key(label) {
                ...
                // When Address Not Encountered, Queue as Incomplete
	...
	fn process_load_instruction(&mut self, expr: &Expression) {
		if let Token::Register(ref reg1) = expr[1] {
			if expr[1].is_general_purpose_register() {
				// Operand 1 is General Purpose Register
				let reg1_num = self.register_name_to_u8(reg1);
				...
				// Operand 1 is *Not* General Purpose Register
				if let Token::Register(ref reg2) = expr[2] {
					let reg2_num = self.register_name_to_u8(reg2);
					...
					match reg1.as_ref() {
						"I" => {
							...
							if self.labels.contains_key(label) {
								...
								// If Address Not Encountered, Queue as Incomplete
								self.queue_incomplete_instruction(expr);
							}
						},
